package com.gyt.managementservice.api.controller;
...
public class UserController {
    ...
    @GetMapping("/getAll")
    @ResponseStatus(HttpStatus.OK)
    public Page<GetAllUserResponse> getAllUsers(
            @RequestParam(defaultValue = "0") int pageNumber,
            @RequestParam(defaultValue = "10") int pageSize) {

        log.info("Received request to retrieve all users page : {}, size: {}", pageNumber, pageSize);
        Page<GetAllUserResponse> allUser = userService.getAllUsers(pageNumber, pageSize);
